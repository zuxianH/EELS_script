...
1. Click **Browse folders** to open your system’s folder-selection window, navigate to your data folder, and confirm the selection to load its scans. Cancelling keeps the current folder. You can also type or paste a path into **Data folder**. The window opens on the computer running the app. If a desktop window is unavailable, an in-app folder browser appears: use **Up**, **Home**, and **Open subfolder**, then click **Use this folder**. Select any number of scans. The first two supported files are selected initially. Temperature labels such as `300 K` and `1000 K` are inferred from filenames where available. Edit labels in **Curve labels**.
